            ...
            $exists2 = mysql_num_rows($query2);
                if($exists2 > 0) //IF there are no returning rows or no existing username
                {
                   while($row2 = mysql_fetch_assoc($query2)) // display all rows from query
                   {
                    ...
                    $result_ref =array();
                    foreach ($output2 as $k_input_uji => $v_input_uji) {
    foreach ($output1 as $k_input_ref1 => $v_input_ref1) {
        foreach ($v_input_ref1 as $k_input_ref2 => $v_input_ref2) {
            // Menghilangkan Simbol pada kalimat 
            $result_uji = preg_replace('/[^a-zA-Z0-9]/',' ', $v_input_uji);
            $hasil_input = explode(" ",$result_uji); 
            // print_r($hasil_input) ;
            // exit;



            $hasil_ref[] = preg_replace('/[^a-zA-Z0-9]/',' ', $v_input_ref2);
            // $hasil_ref = explode(" ",$result_ref); 
        }
    }
}

foreach ($hasil_input as $key_input=>$val_input) {
    if(empty($hasil_input[$key_input])){
        unset($hasil_input[$key_input]);
    }
        // print_r($hasil_input) ;
        // exit;
}
...
foreach ($hasil_input as $key_input=>$val_input) {
    ...
    foreach ($hasil_ref as $key_benar=>$val_benar) {

        // calculate the distance between the input word,
        // and the current word
        // $lev = levenshtein($val_input, $val_benar); //pakai library
        
        $lev = levenshtein($val_input, $val_benar); //pakai algoritma pada lev.php 
       
        // Jika String sudah benar
        if ($lev == 0) {

            $closest = $val_benar;
            $shortest = 0;
            // String uji ditemukan sama dengan string pembanding             
        }
   ...
   $hasil_perbaikan[]= str_replace($val_input, $closest, $val_input);


}

$output2 = implode(" ", $hasil_perbaikan);
$output1 = implode(" ", $hasil_input);
